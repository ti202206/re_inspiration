...
//＊＊＊＊＊＊変更：authFreePathsとinitialIsAuthenticated、isCheckingAuthを受け取る＊＊＊＊＊＊
const Header = ({
    authFreePaths,
    isAuthenticated: initialIsAuthenticated,
    isCheckingAuth: initialIsCheckingAuth,
}) => {
    const [isAuthenticated, setIsAuthenticated] = useState(
        initialIsAuthenticated
    );
    const [user, setUser] = useState(null);
    const [isCheckingAuth, setIsCheckingAuth] = useState(initialIsCheckingAuth); //＊＊＊＊＊＊変更：認証チェックの状態管理＊＊＊＊＊＊
    const navigate = useNavigate();
    const location = useLocation();
